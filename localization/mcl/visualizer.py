...
import localization.utils as utils
import numpy as np
...
class Visualizer(object):
    ...
    def setup_plot(self):
        ...
        self.scat = self.ax0.scatter([], [], c=[], s=self.point_size, cmap="jet", vmin=0, vmax=1)
        ...
        self.ax0.legend()
        colorbar = plt.colorbar(self.scat, ax=self.ax[0])
        colorbar.set_label('Weights')

        # setup ax1
        self.ax_location_err, = self.ax1.plot([], [], c='g')

        self.ax1.set(xlim=[0, len(self.location_gt)], ylim=[0, 0.2])
        ...
        self.ax2.set(xlim=[0, len(self.location_gt)], ylim=[0, 5])
    ...
    def update(self, frame_idx, particles):
        ...
        # set estimated pose
        self.ax_est.set_data(self.location_estimates[self.start_idx:frame_idx + 1, 0],
                             self.location_estimates[self.start_idx:frame_idx + 1, 1])

        # Set x and y data
        skip = 20
        self.scat.set_offsets(particle_xyc[::skip, :2])
        # Set colors
        self.scat.set_array(particle_xyc[::skip, 3] / np.max(particle_xyc[:, 3]))
